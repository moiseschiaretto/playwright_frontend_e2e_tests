...
O `playwright.config.ts` define 4 projetos, validando o comportamento **responsivo** da aplicação em três breakpoints reais:

| Projeto | Viewport | Depende de auth? |
|---|---|---|
| `e2e-setup` | — | executa o login e salva a sessão |
| `desktop` | 1920×1080 | sim (via `dependencies`) |
| `mobile` | Pixel 5 | sim |
| `tablet` | 834×1194 | sim |
...
- `@compact-only` — roda só em `mobile`/`tablet`
